...
exports.getUserStats = exports.getMatches = exports.handleSwipe = exports.getRecommendations = exports.getUsersByIds = exports.verifyGoogleToken = void 0;
...
function checkRateLimit(userId) {
    ...
    rateLimitCache[userKey] = rateLimitCache[userKey].filter(timestamp => (currentTime.getTime() - timestamp.getTime()) < 3600000);
...
function calculateCompatibilityScore(user1, user2) {
    ...
    if (user1.location && user2.location) {
        const distance = calculateDistance(user1.location.lat || 0, user1.location.lng || 0, user2.location.lat || 0, user2.location.lng || 0);
 ...
 */
exports.verifyGoogleToken = (0, https_1.onCall)(async (req) => {
    try {
        ...
        if (!userDoc.exists) {
            ...
            const newUserData = Object.assign(Object.assign({}, userData), { createdAt: admin.firestore.FieldValue.serverTimestamp(), age: null, bio: "", interests: [], location: null, photos: picture ? [picture] : [], isProfileComplete: false });
            ...
            return { success: true, user: Object.assign(Object.assign({}, existingData), userData), isNewUser: false };
        }
    }
    catch (error) {
        console.error("Error verifying Google token:", error);
        throw new https_1.HttpsError("internal", `Authentication failed: ${error}`);
    }
});
/**
 * Get multiple users by IDs, handling Firestore whereIn limit of 30
 */
exports.getUsersByIds = (0, https_1.onCall)(async (req) => {
    const ids = req.data.ids;
    if (!ids || !Array.isArray(ids)) {
        throw new https_1.HttpsError("invalid-argument", "ids array is required");
    }
    if (ids.length === 0) {
 ...
 */
exports.getRecommendations = (0, https_1.onCall)(async (req) => {
    var _a;
    try {
        const userId = (_a = req.auth) === null || _a === void 0 ? void 0 : _a.uid;
        if (!userId) {
            throw new https_1.HttpsError("unauthenticated", "User must be authenticated");
        }
        // Check rate limit
        if (!checkRateLimit(userId)) {
            throw new https_1.HttpsError("resource-exhausted", "Rate limit exceeded. Maximum 30 queries per hour.");
        }
        ...
        throw new https_1.HttpsError("internal", `Failed to get recommendations: ${error}`);
    }
});
/**
 * Handle user swipe and check for matches
 */
exports.handleSwipe = (0, https_1.onCall)(async (req) => {
    var _a;
    try {
        const userId = (_a = req.auth) === null || _a === void 0 ? void 0 : _a.uid;
        if (!userId) {
        ...
        if (!target_user_id) {
            throw new https_1.HttpsError("invalid-argument", "target_user_id is required");
        }
        // Record the swipe
        const swipeData = {
            user_id: userId,
        ...
        throw new https_1.HttpsError("internal", `Failed to handle swipe: ${error}`);
    }
});
/**
 * Get user's matches
 */
exports.getMatches = (0, https_1.onCall)(async (req) => {
    var _a;
    try {
        const userId = (_a = req.auth) === null || _a === void 0 ? void 0 : _a.uid;
        if (!userId) {
        ...
        for (const matchDoc of matches1Snapshot.docs) {
            ...
            if (otherUserDoc.exists) {
                matches.push({
                    ...
                    other_user: Object.assign({ user_id: otherUserId }, otherUserDoc.data()),
        ...
        matches.sort((a, b) => { var _a, _b; return ((_a = b.created_at) === null || _a === void 0 ? void 0 : _a.toMillis()) - ((_b = a.created_at) === null || _b === void 0 ? void 0 : _b.toMillis()); });
        return {
            success: true,
            matches,
        ...
        throw new https_1.HttpsError("internal", `Failed to get matches: ${error}`);
    }
});
/**
 * Get user statistics and rate limit info
 */
exports.getUserStats = (0, https_1.onCall)(async (req) => {
    var _a;
    try {
        const userId = (_a = req.auth) === null || _a === void 0 ? void 0 : _a.uid;
        if (!userId) {
        ...
        throw new https_1.HttpsError("internal", `Failed to get user stats: ${error}`);
    }
});
